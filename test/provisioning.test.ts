...
describe("hub skill tools", () => {
  ...
  it("reads shared-runtime history and rejects out-of-range limits without a request", async () => {
    const history = { hub_id: "hub-1", runtime_group_id: "shared", data: [
      { id: "event:1", kind: "event", actor_email: null, version: "1.2.0" },
      { id: "operation:1", kind: "operation", operation_id: "op-1", status: "failed" },
    ] };
    const fake = await startFakeControlPlane(jsonResponder((request) => request.path === "/v1/hubs/hub-1/skills/history" && request.query.limit === "200" ? { body: history } : undefined));
    ...
    const result = await client.callTool({ name: "thalovant_list_hub_skill_history", arguments: { hubId: "hub-1", limit: 200 } });
    expect(result.isError ?? false).toBe(false);
    expect(JSON.parse(resultText(result))).toEqual(history);
    expect(findRequest(fake, "GET", "/v1/hubs/hub-1/skills/history").headers.authorization).toBe(`Bearer ${API_TOKEN}`);
    const requestCount = fake.requests.length;
    for (const limit of [0, 201, 1.5]) {
      const invalid = await client.callTool({ name: "thalovant_list_hub_skill_history", arguments: { hubId: "hub-1", limit } });
